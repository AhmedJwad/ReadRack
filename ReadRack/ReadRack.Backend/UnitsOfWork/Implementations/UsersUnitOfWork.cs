...
namespace ReadRack.Backend.UnitsOfWork.Implementations
{
    public class UsersUnitOfWork :GenericUnitOfWork<User>, IUsersUnitOfWork
    {
       ...
       => await _usersRepository.CheckRoleAsync(roleName);
        ...
        public override Task<ActionResponse<IEnumerable<User>>> GetAsync(PaginationDTO pagination)
        =>  _usersRepository.GetAsync(pagination);

        public override Task<ActionResponse<int>> GetRecordsNumberAsync(PaginationDTO pagination)
        =>  _usersRepository.GetRecordsNumberAsync(pagination);
